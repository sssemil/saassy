...
export default function UserActions({
  ...
  return (
    <div>
      <h2 style={{ fontSize: 15, marginBottom: 12, color: 'var(--text-secondary)' }}>Actions</h2>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {isFrozen ? (
          <Btn onClick={unfreeze} busy={busy === 'unfreeze'} disabled={disabled('unfreeze')}>
            Unfreeze
          </Btn>
        ) : (
          <Btn
      ...
      {error && (
        <p style={{ marginTop: 12, color: 'var(--text-error)' }}>Error: {error}</p>
      )}
    </div>
  )
}
