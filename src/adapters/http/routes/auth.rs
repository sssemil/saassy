...
#[derive(Deserialize)]
struct RequestPayload { email: String }

#[derive(Deserialize)]
struct ConsumePayload { token: String }
...
async fn consume(State(app_state): State<AppState>, Json(payload): Json<ConsumePayload>) -> AppResult<impl IntoResponse> {
    let auth = app_state.auth_use_cases.clone();
    if let Some(user_id) = auth.consume_magic_link(&payload.token).await? {
        let access = jwt::issue(user_id, &app_state.config.jwt_secret, app_state.config.access_token_ttl)?;
        let refresh = jwt::issue(user_id, &app_state.config.jwt_secret, app_state.config.refresh_token_ttl)?;
